...
# ver 5: use cached distance matrix if existing in order to save effort
# ERROR WHEN READING CACHED DIST MATRIX
...
from config import PPOConfig as cfg # Import Config để lấy đường dẫn
...
class RealDataLoader:
    ...
    def _load_fleet_from_csv(self, truck_csv_path: str):
        """
        Đọc TruckMaster.csv để tạo VehicleType objects
        """
    ...
    def load_day_data(self, order_csv_path: str, truck_csv_path: str) -> ProblemData:
        """
        Load orders, Load Fleet, and receive Matrices (Cached or OSRM).
        """
        ...
        cache_dir = cfg.DISTANCE_TIME_PATH
        ...
        if os.path.exists(cache_path):
            ...
            try:
                # Load CSV
                df_cache = pd.read_csv(cache_path, index_col=0, dtype=str) # Index là NodeID
                df_cache = df_cache.astype(float)
                
                # Reindex để đảm bảo khớp thứ tự với node_ids của đơn hàng hiện tại
                # (Quan trọng: Phòng trường hợp file order bị lọc bớt khách hàng so với lúc tạo matrix)
                ...
                print(f"    -> Loaded and Aligned Matrix Shape: {dist_matrix_meters.shape}")
        ...
        # Tính toán lại Time Matrix dựa trên Fleet Speed (Nhanh, không tốn mạng)
        # print("  > Building Super Time Matrix...")
