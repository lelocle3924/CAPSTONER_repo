...
def one_for_one(data: ProblemData) -> RvrpState:
    """
    Extremely fast initialization for RL Training.
    Gives each customer their own vehicle.
    """
    routes = []
    # Use the cheapest/smallest vehicle
    v_type = min(data.vehicle_types, key=lambda x: x.fixed_cost)
    
    for i in range(1, data.num_nodes):
        # We create a simple route. Repair operators will handle feasibility later.
        r = Route(v_type, [i])
        # Update metrics roughly (or let first ALNS step fix it)
        r.update_centroid(data.coords)
        routes.append(r)
