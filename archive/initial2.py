...
def calculate_route_metrics(route_nodes: list[int], vehicle: VehicleType, data: ProblemData):
    ...
    for node in route_nodes:
        # --- TRAVEL ---
        # Lấy khoảng cách và thời gian từ ma trận
    ...
    time_back = data.time_matrix[prev_node, 0]
...
def one_for_one(data: ProblemData) -> RvrpState:
    ...
    for idx, customer_id in enumerate(data.node_ids):
        customer_idx = idx
        ...
        for vehicle in sorted_fleet:
            ...
            print(f"customer {customer_idx}, status {status}")
