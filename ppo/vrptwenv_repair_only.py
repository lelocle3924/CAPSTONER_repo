...
class VRPTWEnvironmentRepairOnly(gym.Env):
    """
    Ablation experiment environment: PPO only selects repair operators, destroy operators are intelligently selected by ALNS

    This environment maintains the same action space and observation space as the original VRPTWEnvironment,
    but ignores the action input of destroy operators, making it directly compatible with the original model for ablation experiments.
    """
    ...
    def __init__(
        ...
        """
        Initialize VRPTW environment (only select repair operators)
        Args:
        ...
        """
        super(VRPTWEnvironmentRepairOnly, self).__init__()
        ...
        self.destroy_weights = np.ones(
            self.d_op_num, dtype=np.float32
        ...
        # Action space: [destroy operator index(ignored), repair operator index, whether to accept new solution, whether to terminate algorithm]
        # Maintain the same action space as the original environment, but ignore destroy operator selection
        self.action_space = spaces.MultiDiscrete(
            [
                self.d_op_num,  # Destroy operator (will be ignored in this environment)
        ...
        # State space (maintain consistency with original environment, including destroy operator usage history)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.repair_usage[:] = 0
        self.destroy_weights[:] = 1.0  # Reset destroy operator weights
    ...
    def _select_destroy_operator_intelligently(self):
        """
        Intelligently select destroy operator based on weights (roulette wheel algorithm)
        """
        # Calculate probability distribution
        probs = self.destroy_weights / np.sum(self.destroy_weights)
        # Select operator based on probability
        d_idx = self.alns._rng.choice(range(self.d_op_num), p=probs)
        return d_idx

    def _update_destroy_operator_weight(self, d_idx, outcome):
        """
        Update destroy operator weights based on operation results
        Args:
            d_idx: Destroy operator index
            outcome: Operation result (0: new global optimum, 1: better solution, 2: accepted solution, 3: rejected solution)
        """
        self.destroy_weights[d_idx] = (
            self.decay * self.destroy_weights[d_idx]
            + (1 - self.decay) * self.scores[outcome]
        )

    def iterate_repair_only(self, cur_solution, pre_solution, r_idx, accept):
        """
        Execute single ALNS iteration, only select repair operator, destroy operator intelligently selected
        """
        d_idx = None  # Used to record the selected destroy operator index

        if accept == 0:
            cur_solution = pre_solution
        elif accept == 1:
            # Intelligently select destroy operator
            d_idx = self._select_destroy_operator_intelligently()
            d_name, d_operator = self.alns.destroy_operators[d_idx]
            # PPO selects repair operator
        ...
        return pre_solution, cur_solution, d_idx

    def step(self, action):
        """
        Execute one step of ALNS iteration
        Args:
            action: [destroy operator index (ignored), repair operator index, whether to accept new solution, whether to terminate algorithm]
        """
        d_idx, r_idx, accept, stop = (
            action  # d_idx will be ignored, destroy operator through intelligent selection
        )
        ...
        # Execute ALNS iteration (only select repair operator, destroy operator intelligently selected)
        # Note: d_idx parameter is ignored, destroy operator is determined by intelligent selection algorithm
        self.pre_solution, self.current_solution, selected_d_idx = (
            self.iterate_repair_only(
                self.current_solution, self.pre_solution, r_idx, accept
            )
        )
        ...
        # Update destroy operator weights (only when destroy operator is selected)
        if selected_d_idx is not None and accept == 1:
            ...
            # Update weights
            self._update_destroy_operator_weight(selected_d_idx, outcome)
